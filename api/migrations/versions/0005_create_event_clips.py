...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("analytics_event_id", postgresql.UUID(as_uuid=False),
                  sa.ForeignKey("analytics_events.id", ondelete="CASCADE"), nullable=False),
        sa.Column("storage_provider", sa.String(50), nullable=False, server_default="google_drive"),
        sa.Column("storage_ref", sa.String(500), nullable=True,
                  comment="file id no provider, ex: Google Drive fileId"),
        ...
        sa.UniqueConstraint("analytics_event_id", name="uq_event_clips_event"),
    )
